...
- **Welcome Email**: Sends a branded HTML welcome email via SMTP (smtp.masakhegroup.co.za:465) upon user registration. Uses nodemailer with TLS. Email includes feature overview, dashboard CTA button, and trial information. Configured via `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_FROM`, `SMTP_SECURE`, `SMTP_PASSWORD` env vars. Implementation: `server/email.ts`, called from `server/auth.ts` registration endpoint (fire-and-forget, non-blocking).
