...
const C = {
  sb: '#15100C', sbBorder:'rgba(255,240,200,0.08)', main:'#FAFAF7',
  hdrBg:'#FAFAF7', hdrBorder:'#E8E1D5',
  card:'#FFFFFF', cardBorder:'#EBE5D8',
  accent:'#A07B2A', accentDim:'rgba(160,123,42,0.09)',
  textPrimary:'#1C1714', textSec:'#7A7268',
  navText:'rgba(255,240,200,0.45)', navActive:'rgba(255,240,200,0.9)',
  up:'#276749', down:'#9B2226',
};
  ...
  return (
    <div style={{ display:'flex', height:'100vh', fontFamily:"Inter,'Segoe UI',system-ui,sans-serif", overflow:'hidden', fontSize:13 }}>
      ...
      <aside style={{ width:216, flexShrink:0, background:C.sb, display:'flex', flexDirection:'column', borderRight:`1px solid ${C.sbBorder}`, animation:'dv-slideLeft 0.4s ease-out' }}>
        <div style={{ padding:'20px 16px 18px', borderBottom:`1px solid ${C.sbBorder}` }}>
          <div style={{ display:'flex', alignItems:'center', gap:9 }}>
            ...
            <div>
              <div style={{ color:'rgba(255,240,200,0.9)', fontWeight:600, fontSize:13, letterSpacing:'-0.2px' }}>Masakhe</div>
        ...
        <div style={{ padding:'12px 16px', borderTop:`1px solid ${C.sbBorder}`, display:'flex', alignItems:'center', gap:9 }}>
          <div style={{ width:28, height:28, borderRadius:'50%', background:C.accentDim, border:`1px solid rgba(160,123,42,0.3)`, display:'flex', alignItems:'center', justifyContent:'center', color:C.accent, fontSize:11, fontWeight:700 }}>TM</div>
          <div>
            <div style={{ color:'rgba(255,240,200,0.85)', fontSize:12, fontWeight:500 }}>Thabo Mokoena</div>
            <div style={{ color:C.navText, fontSize:10.5 }}>Pro Plan</div>
          </div>
        </div>
      </aside>
      ...
      <main style={{ flex:1, background:C.main, display:'flex', flexDirection:'column', overflow:'hidden' }}>
        ...
        {/* Header */}
        <div style={{ padding:'14px 28px', background:C.hdrBg, borderBottom:`1px solid ${C.hdrBorder}`, display:'flex', alignItems:'center', justifyContent:'space-between', animation:'dv-fadeIn 0.4s ease-out' }}>
          <div>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            ...
            <div style={{ width:32, height:32, borderRadius:7, border:`1px solid ${C.hdrBorder}`, display:'flex', alignItems:'center', justifyContent:'center', color:C.textSec, cursor:'pointer', background:C.card }}>{Ic.bell}</div>
          </div>
        </div>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'22px 28px', display:'flex', flexDirection:'column', gap:18 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12 }}>
            {STATS.map((s, i) => (
              <div key={i} style={{ background:C.card, border:`1px solid ${C.cardBorder}`, borderRadius:10, padding:'16px 18px', boxShadow:'0 1px 3px rgba(28,23,20,0.05)', animation:`dv-fadeUp 0.4s ease-out ${0.15+i*0.08}s both` }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1.55fr 1fr', gap:12 }}>
            <div style={{ background:C.card, border:`1px solid ${C.cardBorder}`, borderRadius:10, padding:'18px 22px', boxShadow:'0 1px 3px rgba(28,23,20,0.05)', animation:'dv-fadeUp 0.4s ease-out 0.5s both' }}>
              ...
              <svg width="100%" viewBox={`0 0 ${CW} ${CH}`} preserveAspectRatio="none" style={{ height:100, display:'block' }}>
                <defs>
                  <linearGradient id="e-rev" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.accent} stopOpacity="0.15"/><stop offset="100%" stopColor={C.accent} stopOpacity="0"/></linearGradient>
                  <linearGradient id="e-exp" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.down} stopOpacity="0.08"/><stop offset="100%" stopColor={C.down} stopOpacity="0"/></linearGradient>
                </defs>
                <path d={smooth(EXP, true)} fill="url(#e-exp)"/>
                <path d={smooth(EXP)} fill="none" stroke={C.down} strokeWidth="1.5"/>
                <path d={smooth(REV, true)} fill="url(#e-rev)"/>
            ...
            <div style={{ background:C.card, border:`1px solid ${C.cardBorder}`, borderRadius:10, padding:'18px 22px', boxShadow:'0 1px 3px rgba(28,23,20,0.05)', animation:'dv-fadeUp 0.4s ease-out 0.6s both' }}>
          ...
          {/* Table */}
          <div style={{ background:C.card, border:`1px solid ${C.cardBorder}`, borderRadius:10, overflow:'hidden', boxShadow:'0 1px 3px rgba(28,23,20,0.05)', animation:'dv-fadeUp 0.4s ease-out 0.7s both' }}>
