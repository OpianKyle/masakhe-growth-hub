...
export function EarthyBold() {
  return (
    <div data-theme="coffee" className="flex h-screen overflow-hidden" style={{ fontFamily:"Inter,'Segoe UI',system-ui,sans-serif", fontSize:13 }}>
      ...
      <aside className="w-[216px] shrink-0 flex flex-col bg-base-200 border-r border-base-300" style={{ animation:'dv-slideLeft 0.4s ease-out' }}>
        ...
        <ul className="menu menu-sm p-2 flex-1 gap-0.5 mt-1">
          {NAV.map((n,i) => (
            <li key={i} style={{ animation:`dv-fadeUp 0.35s ease-out ${i*0.05}s both` }}>
              <a className={n.active ? 'active font-semibold' : 'text-base-content/40 hover:text-base-content/70 hover:bg-base-300/50'}>
        ...
        <div className="px-4 py-3 border-t border-base-300 flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-full bg-primary/20 flex items-center justify-center text-primary text-[11px] font-semibold">TM</div>
          <div>
            ...
            <div className="col-span-3 card bg-base-200 border border-base-300 shadow-none" style={{ animation:'dv-fadeUp 0.4s ease-out 0.44s both' }}>
              <div className="card-body p-5">
                ...
                <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ height:H }}>
                  <defs>
                    <linearGradient id="cf-r" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#c8a96e" stopOpacity="0.22"/><stop offset="100%" stopColor="#c8a96e" stopOpacity="0"/></linearGradient>
                    <linearGradient id="cf-e" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#ef4444" stopOpacity="0.1"/><stop offset="100%" stopColor="#ef4444" stopOpacity="0"/></linearGradient>
                  </defs>
                  <path d={sp(EXP,true)} fill="url(#cf-e)"/>
                  <path d={sp(EXP)} fill="none" stroke="#ef4444" strokeWidth="1.5"/>
                  <path d={sp(REV,true)} fill="url(#cf-r)"/>
                  <path d={sp(REV)} fill="none" stroke="#c8a96e" strokeWidth="1.8"/>
                </svg>
