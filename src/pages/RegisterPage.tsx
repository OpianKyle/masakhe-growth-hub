...
import { ArrowRight, ArrowLeft, Building2, User, FileText, MapPin, Phone, Check, Lock } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (result.ok) {
      toast.success("Registration successful! Welcome to Masakhe.");
      navigate("/dashboard");
    } else {
        ...
        <div className="flex items-center justify-between mb-12">
          {steps.map((step, i) => (
            <div key={step.title} className="flex items-center">
              ...
              {i < steps.length - 1 && (
                <div className={`h-0.5 w-8 md:w-12 mx-2 ${i < currentStep ? "bg-primary" : "bg-border"}`} />
              )}
            </div>
          ))}
        </div>
            ...
            {currentStep === 5 && (
              <div className="space-y-6 text-center">
                ...
                <Button variant="hero" size="lg" className="mt-4 w-full" onClick={handleSubmit} disabled={loading}>
                  {loading ? "Creating account..." : "Complete Registration"} <ArrowRight className="ml-2 h-5 w-5" />
                </Button>
              </div>
            )}
          </motion.div>
