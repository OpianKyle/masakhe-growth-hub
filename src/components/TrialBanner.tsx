...
import { Clock, X, AlertTriangle, Lock } from "lucide-react";
...
export default function TrialBanner() {
  ...
  useEffect(() => {
    ...
    if (status === "TRIAL") {
      ...
      if (!sessionStorage.getItem(shownKey) && !popupShown) {
  ...
  return (
    <>
      ...
      {status === "TRIAL" && days > 0 && popupShown && (
        <Dialog open={popupShown} onOpenChange={setPopupShown}>
          <DialogContent className="max-w-md">
            <div className="text-center space-y-4 py-2">
              ...
              <div className="flex gap-3">
                <Button className="flex-1" onClick={() => { setPopupShown(false); navigate("/dashboard/billing"); }}>
                  Subscribe Now
                </Button>
                <Button variant="outline" className="flex-1" onClick={() => setPopupShown(false)}>
                  Continue Trial
                </Button>
              </div>
            </div>
