...
accountsRouter.post("/:workspaceId/accounts/oauth/meta/start", requireWorkspaceRole("owner", "admin"), (req, res) => {
  ...
  const authUrl = `https://www.facebook.com/v19.0/dialog/oauth?client_id=${process.env.META_APP_ID}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=pages_show_list,pages_read_engagement,instagram_basic,instagram_content_publish&state=${statePayload}`;
  res.json({ authUrl });
});
